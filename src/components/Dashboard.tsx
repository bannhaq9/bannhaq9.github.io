...
export default function Dashboard({ 
  ...
  // Count properties in each ward for clean bar charts
  ...
  return (
    <div className="space-y-8 p-4 md:p-6 lg:p-8 max-w-7xl mx-auto id-dashboard-root">
      ...
      {/* Numerical Bento Grid of metrics */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden group">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="p-1.5 bg-red-50 text-brand-primary rounded-lg">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden group">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="p-1.5 bg-yellow-50 text-amber-600 rounded-lg">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden group">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="p-1.5 bg-green-50 text-brand-green rounded-lg">
      ...
      {/* Graphical Breakdown & Logs Feed */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* Ward Distribution Visual */}
              ...
              logs.map((log) => {
                ...
                return (
                  <div key={log.id} className="flex items-start gap-2.5 text-xs text-slate-600 border-b border-slate-50 pb-2">
                    ...
                    <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${badgeColor} uppercase shrink-0`}>
                      {log.type.replace("_", " ")}
                    </span>
      ...
      {/* Tab selection for management databases */}
      <div className="flex border-b border-slate-200 mt-2 select-none">
        <button
          onClick={() => setActiveSection("leads")}
          className={`pb-3 text-sm font-bold uppercase tracking-wider relative px-4 transition-all cursor-pointer ${
            activeSection === "leads" 
        ...
        <button
          ...
          className={`pb-3 text-sm font-bold uppercase tracking-wider relative px-4 transition-all ml-4 cursor-pointer ${
      ...
      {/* Structured Management Database Grid */}
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        {activeSection === "leads" ? (
          <div>
            {/* Table header menu for leads */}
                ...
                <div className="flex rounded-lg border border-slate-200 p-1 bg-white">
                  {(["all", "new", "contacted", "closed"] as const).map((tag) => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-md text-slate-600 text-[11px] font-semibold transition-all cursor-pointer ${
                        leadFilter === tag 
                  ...
                  {filteredLeads.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="py-12 text-center text-xs text-slate-400 italic">
                        Không tìm thấy dữ liệu đăng ký tương ứng...
                      </td>
                    </tr>
                  ) : (
                      ...
                      return (
                        <tr key={lead.id} className="border-b border-slate-50 hover:bg-slate-50/50 text-xs text-slate-700 transition-colors">
                          ...
                          <td className="py-3.5 px-5">
                            ...
                            {lead.propertyId && (
                              <span className="text-[10px] text-slate-400 font-mono">BĐS ID: {lead.propertyId}</span>
                            )}
                          </td>
                          ...
                          <td className="py-3.5 px-5 text-right space-x-1.5 whitespace-nowrap">
                            {lead.status === "new" && (
                              <button
                                ...
                                title="Đánh dấu đã liên hệ cuộc gọi"
                              >
          ...
          <div>
            {/* Table header menu for properties */}
            <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                ...
                <p className="text-[11px] text-slate-500 mt-1">
                  {adminMode 
                    ? "Bạn đang ở giao diện Quản trị. Có thể sửa thông tin hoặc xóa bài đăng rác." 
                    : "⚠️ Sử dụng quyền Quản trị (Admin ON ở góc phải) để chỉnh sửa hoặc xóa tin đăng."}
                </p>
              </div>

              <div className="flex flex-wrap items-center gap-2">
                {/* Keyword search in properties table */}
                <div className="relative">
                  <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
                  <input
                    type="text"
                    placeholder="Tìm tiêu đề, tên đường, phường..."
                  ...
                  {filteredProperties.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="py-12 text-center text-xs text-slate-400 italic">
                        Không tìm thấy bài viết bất động sản thích hợp...
                      </td>
                    </tr>
                  ) : (
                    filteredProperties.map((p) => {
                      return (
                        <tr key={p.id} className="border-b border-slate-50 hover:bg-slate-50/50 text-xs text-slate-700 transition-colors">
                          <td className="py-3.5 px-5">
                            <div className="font-bold text-slate-900 line-clamp-1 max-w-[280px]" title={p.tieu_de}>
                              {p.tieu_de}
                            </div>
                            <div className="text-[10px] text-slate-400 font-mono mt-0.5">ID: {p.id} · Đăng ngày: {p.created_at ? new Date(p.created_at).toLocaleDateString("vi-VN") : "Hệ thống"}</div>
                          </td>
                          <td className="py-3.5 px-5">
                          ...
                          <td className="py-3.5 px-5 space-y-0.5">
                            ...
                            <div className="text-[10px] text-slate-500 font-medium">{p.bedroom} PN · {p.nhavesinh} WC · Hướng {p.direction}</div>
                          </td>
                          <td className="py-3.5 px-5">
                            <div className="font-black text-amber-600 italic tracking-tight">{p.price} TỶ</div>
                            {p.oldPrice && p.price !== p.oldPrice && (
                              <div className="text-[9px] text-slate-400 line-through">Cũ: {p.oldPrice} tỷ</div>
                            )}
                          </td>
                          <td className="py-3.5 px-5">
                            <span className="font-bold text-slate-800 text-[10px] uppercase bg-slate-100 px-2 py-0.5 rounded">
                              {p.phaply || "Sổ hồng riêng"}
                            </span>
                          </td>
                          <td className="py-3.5 px-5 font-bold font-mono text-slate-600">
                            {p.views || 0} lượt
                          </td>
                          <td className="py-3.5 px-5 text-right space-x-1.5 whitespace-nowrap">
                            {adminMode ? (
                              <>
                                ...
                                <button
                                  onClick={() => {
                                    if (window.confirm(`Bạn có chắc chắn muốn xóa vĩnh viễn bài đăng: "${p.tieu_de}"?`)) {
                                  ...
                                  title="Xóa vĩnh viễn tin rao này"
                                >
